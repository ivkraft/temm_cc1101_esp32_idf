...
void gfx_init(void) {
    ...
    s_fb = (uint16_t *)heap_caps_malloc(n * sizeof(uint16_t), MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
...
void gfx_char(int x, int y, char ch, uint16_t fg) { draw_char_core(x, y, ch, fg, false, 0); }

void gfx_char_bg(int x, int y, char ch, uint16_t fg, uint16_t bg) { draw_char_core(x, y, ch, fg, true, bg); }
